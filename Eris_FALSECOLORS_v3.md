...
This paper introduces FALSECOLORS, a noncryptographic security primitive for anyone who needs to openly carry or publish text with a hidden message. It protects sensitive textual documents through structure-preserving semantic domain transformation. Unlike encryption, which renders data incomprehensible and thereby signals its value, FALSECOLORS produces output that is fully comprehensible, domain-coherent, and self-consistent in an unrelated cover domain. The sensitive content is recoverable only through possession of compact key artifacts. Without the keys, cover documents stand alone as legitimate documents in their surface domains.

**Threat model scope.** The deniability claim is operational against time-constrained pattern-matching adversaries: customs officers and litigation reviewers under deadline, automated content classifiers running keyword filters, cloud providers performing routine log triage, and rubberhose interrogations where the artifact itself is the only evidence available. The claim is **not** operational against adversaries who invoke a frontier-class LLM as a detection tool. Empirical evaluation in the v4 LACH companion paper (`Eris_FALSECOLORS_v4_LACH.md`) measures the realized adversary advantage `Adv_D` against named contemporary detectors and finds that Method 4 covers in the current cohort are reliably identified as transformed source content under source-domain-identification scoring. Section 12 of this paper specifies the full threat-model boundary; users deploying FALSECOLORS against an adversary class that includes frontier LLM detection should consult the v4 evaluation before relying on the deniability property.
...
**Deniability (information-theoretic, scoped).** The Caudle Semantic Secrecy Theorem (Section 2.13) establishes that a cover document under a uniformly-random domain isomorphism provides zero information about the source document beyond the prior. This is an information-theoretic statement about the mapping's key space. Empirically, the realized deniability against a specific adversary depends on the adversary's strategy:

- Against passive observers, automated content classifiers, statistical analyses based on n-gram distributions or token-adjacency divergences (Caudle Distance, Section 2.15), and human domain experts performing time-constrained review, Method 4 covers are operationally indistinguishable from native cover-domain text. This is the threat model the v3 paper was written for and where FALSECOLORS is deployable today.

- Against adversaries who invoke a frontier-class LLM as a detection tool, the v4 LACH evaluation (`Eris_FALSECOLORS_v4_LACH.md`) measures realized adversary advantage `Adv_D` near 1.0 across the v3 cohort. Frontier LLMs identify Method 4 covers as transformed source content via document-genre cues, residual vocabulary leak, and structural patterns that survive vocabulary substitution. The deniability claim does not extend to this adversary class with the v3 implementation.

The Caudle Theorem holds under its stated assumptions; the assumption gap between "uniformly-random isomorphism" and "Method 4's locally-LLM-generated isomorphism" is where the operational deniability against frontier detectors is lost. Section 13 enumerates the failure modes; the v4 paper's Section 7.6 documents them empirically.
